...
struct QR_Code
{
    ...
    int check_if_valid(vector<int> cb, vector<int> lb, vector<int> ct, vector<int> lt, int *qb, int *db)
    {
        ...
        cout << "............." << endl;
        cout << cb == this->cb << " " << lb == this->lb << " " << ct == this->ct << " " << lt == this->lt << " " << equal(qb, qb + 4, this->qb) << " " << equal(db, db + 2, this->db) << endl;
